					...
					<table class="table table-bordered"> 
						...
						<nav>
							<ul class="pagination pagination-lg">
								...
								<li><a href="${path}/ssUsers&name=${name}" >首页</a></li>
								<c:if test="${page.hasPreviousPage}">
								<li><a href="${path}/ssUsers?pn=${page.pageNum - 1}&name=${name}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
								</c:if>
							...
							<c:if test="${page_Num != page.pageNum }">
								<li><a href="${path}/ssUsers?pn=${page_Num}&name=${name}">${page_Num}</a></li>
							</c:if>
						</c:forEach>
								<c:if test="${page.hasNextPage}">
								<li><a href="${path}/ssUsers?pn=${page.pageNum+1}&name=${name}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
								</c:if>
								<li> <a href="${path}/ssUsers?pn=${page.pages}&name=${name}">末页</a></li>
							</ul>
						</nav>
						</div>
			</div>
